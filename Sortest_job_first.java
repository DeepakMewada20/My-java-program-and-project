...
public class Sortest_job_first {
    ...
    void print(List<Processs> processes,List<Time> timing){
        ...
        for (int i=0;i< processes.size();i++){
            System.out.print(processes.get(i).process_id+"\t\t"+processes.get(i).aroval_time+"\t\t"+processes.get(i).burest_time+"\t\t");
            System.out.println(timing.get(i).complition_time+"\t\t"+timing.get(i).turn_around_time+"\t\t"+timing.get(i).waiting_time);
        }
